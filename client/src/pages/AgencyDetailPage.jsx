      ...
      {agents.length > 0 && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {agents.map((agent) => (
              <div key={agent._id} className="glass-panel p-6 rounded-3xl border border-slate-200 hover:border-amber-500/50 transition-all duration-300 group flex flex-col justify-between bg-gradient-to-b from-slate-900/80 to-slate-950/80 hover:shadow-lg hover:shadow-amber-500/10">
                <div className="flex items-center space-x-4 mb-4">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <h4 className="text-base font-extrabold text-slate-900 group-hover:text-amber-400 transition-colors truncate">{agent.name}</h4>
                ...
                <div className="pt-4 border-t border-slate-200/80 space-y-2.5 mt-auto">
                  <div className="flex items-center space-x-3 text-xs text-slate-600 group/item">
                    <div className="p-1.5 rounded-md bg-slate-100/80 group-hover/item:bg-amber-500/20 group-hover/item:text-amber-400 transition-colors">
                ...
                <div className="mt-5">
                  <a href={`mailto:${agent.email}`} className="block text-center w-full py-2.5 rounded-xl bg-slate-100/80 border border-slate-300 text-slate-900 text-xs font-bold hover:bg-amber-500 hover:text-slate-950 hover:border-amber-500 transition-all shadow-sm">
                    Contact Agent
                  </a>
                </div>
              </div>
