...
const LiveChatModal = ({ agent, property, isOpen, onClose }) => {
  ...
  const noReplyTimeoutRef = useRef(null);
  ...
  const handleSendMessage = async (textCustom) => {
    ...
    if (isExpertReq) {
      ...
      if (noReplyTimeoutRef.current) clearTimeout(noReplyTimeoutRef.current);

      connectionTimeoutRef.current = setTimeout(() => {
        setShowPendingWaitMsg(true);
      }, 500);

      // If human agent does not reply within 2 minutes (120,000 ms), AI Agent activates with an automated suitable reply
      noReplyTimeoutRef.current = setTimeout(() => {
        ...
        setMessages((prev) => {
          const autoReplyMsg = {
            ...
            receiverId: user._id,
            propertyId: property?._id,
            text: `Thank you for your patience! Our expert agent is currently assisting other clients or away. I am active and ready to assist you right now—feel free to ask me about property details, pricing, scheduling an inspection, or submitting an offer!`,
            createdAt: new Date().toISOString()
          };
